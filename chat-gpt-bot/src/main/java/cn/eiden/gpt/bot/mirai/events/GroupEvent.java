...
public class GroupEvent extends SimpleListenerHost {
    ...
    private void handlerGroup(GroupMessageEvent event){
        ...
        // 如果消息中提及了机器人
        if (atMessage != null && atMessage.getTarget() == event.getBot().getId()) {
            ...
            ChatCompletionApi.sendMessageWithCallback(new GroupMessageGptCallback(event.getGroup(), sendId), message, contextMessage.toArray(new String[0]));
